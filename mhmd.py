...
R = '\033[1;31m'
own_id ="6319474410"
tele_bot='6853086896:AAGIlkVo2vbJX5FWPT4gRGwW7cestAqLUrE'
ch='qwertyuioplkjhgfdsazxcvbnm1234567890'
def create():
	global created
	global failed
	user=str(random.choice([get_first_name().lower(),get_full_name().replace(' ','').lower(),get_last_name().lower()]))+str(''.join(random.choice('1234567790') for alex in range(int(random.randint(2,4)))))+str(random.choice('qwertyuioplkjhgfdsazxcvbnm')[0])+str(''.join(random.choice(ch) for i in range(int(random.randint(6,11)))))
	...
	if '"comment":"Exists"' in str(decoded_data):
		...
		created+=1
		with open('safeum.txt', 'a') as file:
					file.write(f'{user}\n')
		y = requests.post(f"""https://api.telegram.org/bot{tele_bot}/sendmessage?chat_id={own_id}&text={user}""");
